...
--   porque el DO $$ block del backfill tiene un edge case en
--   multi-statement batch: la función PL/pgSQL se compila
--   upfront (CREATE FUNCTION o DO $$) y al ejecutarse NO ve las
--   columnas creadas en el mismo `client.query(sql)` que contiene
--   la CREATE TABLE. Dividiendo en archivos separados, el runner
--   los ejecuta en transactions distintas y el backfill ve el
--   schema ya commiteado.
--
...
BEGIN
  ...
  -- 3) Farms únicos por (client_id, name) — farms sin client_name
