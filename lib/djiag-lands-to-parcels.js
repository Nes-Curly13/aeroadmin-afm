...
/**
 * SQL parametrizado para UPSERT. Conserva las columnas EXCLUSIVAS del
 * legacy import (spray_geom, drone_model_code, spray_width_m, etc.) —
 * solo escribe las que vienen de la API de DJI.
 *
 ...
 *
 * Decisión de diseño (2026-07-30, fix "is_orchard inconsistente con DJI"):
 *   - Antes: `is_orchard` y `field_type` NO estaban en DO UPDATE SET
 *     (preservaban el valor del legacy import — `parameter.tree_spray_selector`).
 *   - Bug: 288 parcelas (24% del total) tenían is_orchard=true en BD pero DJI
 *     las clasifica como PLANT_LAND (farmland). El operador veía una cosa en
 *     DJI SmartFarm Web y otra en nuestro panel.
 *   - Causa probable: `parameter.tree_spray_selector=1` se interpretaba como
 *     "es orchard" cuando en realidad significa "este vuelo se hizo con modo
 *     tree-spray" (un farmland también puede usar ese modo).
 *   - Fix: is_orchard y field_type AHORA se actualizan desde la API de DJI
 *     (landType === 'Orchards' → true, sino false). DJI es la fuente de verdad.
 *   - `land_name` también se actualiza: el legacy import no lo poblaba, la API sí.
 *   - Lo que sigue protegido (NO se toca en DO UPDATE SET): spray_geom,
 *     drone_model_code, spray_width_m, work_speed_mps, optimal_heading_deg,
 *     radar_height_m, edge_offset_m, obstacle_offset_m, climb_height_m,
 *     no_spray_zone_m2, droplet_size, sweep_direction, uses_side_spray,
 *     declared_area_ha, spray_area_m2, raw_geometry, raw_parameter, raw_waypoint,
 *     waypoints, waypoint_count, reference_point, deleted_at, fetched_at.
 *
