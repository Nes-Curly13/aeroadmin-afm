 ...
 *   INSERT ... ON CONFLICT (fumigation_date, source) WHERE parcel_id IS NULL
 *   DO UPDATE SET <solo campos no-identidad>
 *   RETURNING <todas las columnas + (xmax = 0) AS inserted>
 *
 * El conflict target matchea el partial unique index
 * `uq_dji_fumigations_aggregate` (definido en la migration 20260619140000).
 *
 * El caller pasa 10 valores en el orden definido en `paramsToPgArray`.
 * `parcel_id` es NULL para fumigaciones aggregate (no tenemos mapeo finca).
 *
 * `product_used` y `drone_code_used` son NULL por ahora (DJI no expone).
 *
 * El `RETURNING` agrega el truco de idempotencia `(xmax = 0) AS inserted`:
 *   - En un INSERT nuevo, `xmax` es 0 → `inserted = true`.
 *   - En un UPDATE por ON CONFLICT, `xmax` es la txn actual (no-zero)
 *     → `inserted = false`.
 * Eso permite al caller (scripts/upsert-fumigations-from-djiag.js)
 * registrar el evento 'created' en `fumigation_audit_log` SOLO cuando la
 * fila es nueva. Re-correr la pipeline N veces no duplica entradas de
 * audit (sprint feat/pipeline-audit-integration 2026-08-22).
 */
const UPSERT_SQL = `
...
RETURNING id, parcel_id, fumigation_date, product_used, dose_l_per_ha,
          ...
          product_registered_ica, pilot_license, category_id,
          deleted_at, flight_ids, source,
          (xmax = 0) AS inserted
`;
