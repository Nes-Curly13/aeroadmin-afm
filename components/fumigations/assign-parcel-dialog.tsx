 ...
 *
 * UX:
 *   - Botón "Asignar parcela" → `Dialog` (base-ui): focus trap, Esc,
 *     backdrop, títulos accesibles.
 ...
 *
 * UI-O1 (auditoría UI 2026-09-21): migrado del `<dialog>` nativo al
 * primitivo `Dialog` para unificar overlays y corregir el backdrop muerto
 * (`backdrop:bg-foreground/40` no existía como variante Tailwind).
 */
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Asignar parcela</DialogTitle>
          <DialogDescription>
            {note ?? "Buscá la parcela a la que corresponde esta fumigación."}
          </DialogDescription>
        </DialogHeader>
            ...
            options.map((p) => (
              <li key={p.id} className="border-b border-border last:border-b-0">
                <button
                  ...
                  className="flex w-full items-start gap-2 px-3 py-2 text-left hover:bg-muted focus-visible:bg-muted focus-visible:outline-none disabled:opacity-60"
                >
